...
class NodeLevelDataset(incremental_graph_trans_):
    def __init__(self,name='ogbn-arxiv',IL='class',default_split=False,ratio_valid_test=None,args=None):
        ...
        if name[0:4] == 'ogbn':
            ...
            data = CoraFullDataset(raw_dir=args.ori_data_path)
            graph, label = data[0], data[0].dstdata['label'].view(-1, 1)
        elif name in ['reddit','Reddit','Reddit-CL']:
            data = RedditDataset(self_loop=False, raw_dir=args.ori_data_path)
            ...
            data = AmazonCoBuyComputerDataset(raw_dir=args.ori_data_path)
            graph, label = data[0], data[0].dstdata['label'].view(-1, 1)
        else:
            print('invalid data name')
        graph, label = self.standardize(graph, label)
        ...
        for c in cls_sizes:
            if cls_sizes[c] < 3:
                invalid_cls.append(c)
                cls.remove(c) # remove classes with less than 3 examples, which cannot be split into train, val, test sets
        cls_map = {cls[i]: i for i in range(len(cls))}
        for i in range(len(invalid_cls)):
            cls_map[invalid_cls[i]] = i+len(cls)
        for i in range(label.shape[0]):
            label[i,0] = cls_map[int(label[i,0])]
        n_cls = len(cls)
        cls = [i for i in range(n_cls)]
        cls_id_map = {i: list((label.squeeze() == i).nonzero().squeeze().view(-1, ).numpy()) for i in cls}
        if default_split:
    ...
    def standardize(self, graph, label):
        sg = SparseGraph(graph.adjacency_matrix(scipy_fmt='csr'), graph.ndata['feat'].numpy(), labels=label.numpy())
        sg.standardize()
        g_res = dgl.from_scipy(sg.adj_matrix)
        g_res.ndata['feat'] = torch.tensor(sg.attr_matrix)
        l_res = torch.tensor(sg.labels)
        return g_res, l_res
