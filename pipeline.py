...
def pipeline_class_IL_no_inter_edge(args, valid=False):
    ...
    cls = [list(dataset.tr_va_te_split.keys())[i:i+args.n_cls_per_task] for i in range(0, len(list(dataset.tr_va_te_split.keys())), args.n_cls_per_task)] 
    args.task_seq = cls
    ...
    if args.method == 'FTF_ER':
        life_model_ins = life_model.NET(model, task_manager, args, dataset) if valid else None
