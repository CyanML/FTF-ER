...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='FTF_ER')
    parser.add_argument("--dataset", type=str, default='CoraFull-CL', help='Reddit-CL, Arxiv-CL, CoraFull-CL, Amazon-CL')
    parser.add_argument("--gpu", type=int, default=1, help="which GPU to use.")
    parser.add_argument("--seed", type=int, default=555, help="seed for exp")
    ...
    parser.add_argument('--method', type=str,
                        choices=["FTF_ER"], 
                        default="FTF_ER", help="baseline continual learning method")
    ...
    parser.add_argument('--ftfer_args', type=str2dict, default={'budget': [60], 'sampler': ['mix'], 'beta': [0.5]},
                        help='sampler options: mix, mix_plus')
    ...
    parser.add_argument('--ILmode', default='classIL',choices=['classIL'])
    ...
    parser.add_argument('--ori_data_path', type=str, default='./data/raw_data', help='the root path to raw data')
    parser.add_argument('--data_path', type=str, default='./data/processed_data', help='the path to processed data (splitted into tasks)')
    parser.add_argument('--result_path', type=str, default='./FTF_ER_results', help='the path for saving results')
    ...
    method_args = {'FTF_ER': args.ftfer_args}
        ...
        if os.path.isfile(f'{args.result_path}/{name}.pkl') and not args.overwrite_result:
            ...
            for ite in range(args.repeats):
                ...
                try:
                    ...
                    end_time = time.time()
                    print('Time taken for training = {} sec'.format(end_time - start_time))
    ...
    if args.perform_testing:
        ...
        aps = []
        afs = []
        mkdir_if_missing(f'{args.result_path}/test_log/')
        for ite in range(args.repeats):
            ...
            # save the result of testing
            with open(f'{args.result_path}/test_log/{config_name}.txt','a') as f:
                f.write('times:'+str(ite))
                f.write('\nAP:{}, AF:{}\n'.format(AP_test, AF_test))
            aps.append(AP_test)
            afs.append(AF_test)
        with open(f'{args.result_path}/test_log/{config_name}.txt','a') as f:
            f.write('\nmeanAP:{}, std:{} | {}±{}\n'.format(round(np.mean(aps),1), round(np.std(aps),1), round(np.mean(aps),1), round(np.std(aps),1)))
            f.write('\nmeanAF:{}, std:{} | {}±{}\n'.format(round(np.mean(afs),1), round(np.std(afs),1), round(np.mean(afs),1), round(np.std(afs),1)))
